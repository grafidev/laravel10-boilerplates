...
function submit() {
    form.put(route('dashboard.productPrices.update', { product: props.product.id, price: props.price.id }), {
        onSuccess: () => {
            close();
            form.reset();
...
<template>
    <ModalForm :show="show" @close="close" title="Modifier un prix" @submit="submit" :processing="form.processing">
        <ProductPriceFields :product="props.product" :form="form"  :settings="props.settings" />
    </ModalForm>
</template>