...
const createTerminal = ref(false);

watch(() => filterForm.search, value => {
    router.get(route('dashboard.station_terminals.index', { station: props.station.data.id }) + window.location.hash, { search: value }, {
        preserveState: true,
        preserveScroll: true,
    })
}, { immediate: true });
    ...
    <DashboardLayout title="Informations sur la station">
        <template #header>
            <DashboardBreadcrumb title="Informations sur la station">
                ...
                <BreadcrumbItem>Terminaux</BreadcrumbItem>
            </DashboardBreadcrumb>
        </template>

        <CreateTerminal :station="props.station.data" :show="createTerminal == true" @close="createTerminal = false"/>

        <div class="space-y-2">
            <StationTabs :station="props.station.data">
                <FormCard title="Terminaux">
                    <template #actions>
                        <PrimaryButton @click.prevent="createTerminal = true">Ajouter un terminal</PrimaryButton>
                    </template>

                    <TerminalsTable :station="props.station.data" :perpage="props.perpage" :terminals="props.terminals" />
                </FormCard>
            </StationTabs>
        </div>
    </DashboardLayout>
