...
const onChangePerpage = page => {
    changePerPage(page, route('dashboard.admin_roles.index'));
};

watch(search, (value) => {
    router.get(route('dashboard.admin_roles.index'), { search: value }, {
        preserveState: true,
        replace: true,
    });
});
    ...
    <Table>
        ...
        <Tbody>
            ...
            <Tr v-for="role in roles.data" :key="role.id">
                ...
                <Td>
                    <PrimaryButton v-if="auth.permissions.includes('edit_role')" @click.prevent="router.visit(route('dashboard.admin_roles.edit', {admin_role: role.id}))">
                        Modifier
                    </PrimaryButton>
                </Td>
            </Tr>
