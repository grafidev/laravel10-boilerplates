...
<template>
    <Modal :show="props.show" @close="close">
        <ModalContent title="Historique de validation">
            <NoRecords class="flex items-center justify-center" :colspan="1" title="Aucun élément enregistré pour le moment" v-if="props.endowment.validationHistories.length == 0" />

            <ol class="relative border-l border-gray-200 dark:border-gray-700" v-if="props.endowment.validationHistories.length">                   
                <li class="mb-10 ml-6" :key="history.id" v-for="history in props.endowment.validationHistories">
